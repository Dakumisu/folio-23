...
// There's only one IntersectionObserver instance for all the components
 ...
 * @param triggerLeaveOnDestroy - Trigger the onLeave callback when the component is destroyed
 *
...
export function useIntersectionObserver({
	...
	let _rootObserver = document.body;
	...
	onMounted(() => {
		...
		// Unref the root element (in case it's a ref)
		root = unref(root);

		if (singletonObserver) _observer = singletonObserver;
		...
		if (!singletonObserver) singletonObserver = _observer;
